...
It is possible to view the access matrix using GraphViz (for now, only works for IPv4), e.g.:
```
$ dist/build/fffuu/fffuu iptables-save | sed -nur -f tograph.sed | dot -Tpng | feh -
```
The dot file generated by tograph.sed also contains annotations for generating tex code.
Getting dot2tex to correctly position and dimension the nodes requires some tewaking, e.g.:
```
$ function d2t { dot2tex --graphstyle='scale=.5, every node/.style={scale=.7,align=center}' --usepdflatex $@; }; \
  dist/build/fffuu/fffuu iptables-save |\
  sed -nur -f tograph.sed | dot -Txdot |\
  sed -n '/label/{:a;/\\$/{n;b a;};s/^.*$/_ldraw_="F 14 11 -Times-Roman c 7 -#000000 T 552.39 157.42 0 148 29"/;};p' |\
  d2t --preproc | d2t | pdflatex --jobname am --
```
(You will likely at least want to adjust the sizes from the first line.)
