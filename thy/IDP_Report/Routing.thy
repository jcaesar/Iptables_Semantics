...
text\<open>Note how all types that somehow include an IP address are parametrized with @{typ "'a::len"}. 
This is used to be able to use a single implementation for IPv4 and IPv6.
Depending on the IP protocol that is used, @{typ "'a"} is set to the length of the IP address in bits, @{typ 32} or @{typ 128} respectively.\<close>
...
in the sense that those definitions simply do not make sense if the criteria re not verified.
The criteria are:
\begin{enumerate}
  \item The prefixes have to be valid, i.e., 0 in bits exceeding their length. \\
    Example: \texttt{127.0.0.0/8} is valid, but \texttt{127.0.0.1/31} is not.
  \item There has to be a default rule, i.e. one with prefix length 0.
    With the condition above, that implies that all its prefix bits are zero and it thus matches any address.
  \item \label{crit:rtblsort} The entries have to be sorted by prefix length and metric.
  \item The sort order has to be unique.
\end{enumerate}
The validity criteria can formally be expressed in the following way:
\<close>
lemma
...
text\<open>(We show the specialized version for @{typ 32} bit since it is a lot easier to read.)\<close>
...
text\<open>We elude the full definition of criterion \ref{crit:rtblsort} since it only asks 
if the routing table is @{const sorted} lexicographically on prefix length and metric, i.e. \ref{crit:rtblsort} by the following key:%
\footnote{@{const LinordHelper} is a construct to build a lexicographic ordering from two types with linear orders.
...
text\<open>We feel that it is necessary to point out that the definition of @{const valid_prefix}
forbids prefixes that set bits to one behind their prefix length, as intended.
...
Since making weaker assumptions is not harmful, we went for this definition because we deemed it more beautiful than others.
\<close>
...
text\<open>If we use the \texttt{ip route}\todo{cite doc} command to inspect the routing table on a real linux system,
...
\begin{itemize}
  \item This route has a \texttt{proto} entry \todo{}
  \item This route has a \texttt{scope} entry \todo{}
  \item This route has a source address hint.
    ...
    abstracting this is obvious.
    \todo{cite http://linux-ip.net/html/routing-saddr-selection.html}
\end{itemize}
...
In short, the reason here is that routing table selection can be done with an entire separate matching language.
Moreover, the matches can draw upon information from matches in previous matches in the \texttt{mangle} table of IPtables\todo{cite something?}.
...
This question is immediately answered by the following simple algorithm:\<close>
lemma
...
For each rule, its output port and the set of IP addresses that match it, under consideration of previous matches, is emitted.
\<close>
text\<open>For the sake of simplicity, @{const routing_port_ranges} inserts one entry per routing rule.
...
text\<open>The definition @{const routing_ipassmt_wi} simply takes the list returned by @{const routing_port_ranges},
 and, for each distinct port, constructs the union of all sets that are forwarded to that port.
\<close>text\<open>
...
\todo{See sec:foobar for an example where this is important}
\<close>
